...
|---|---|---|---|---|---|---|
| 1 | TVGSUOS | `TVGSUOS` (this repo) | `theverifiedgroup.org` (not wired yet) | Reads/writes the KOS Supabase project — no dedicated database | Phase 0 — docs only | Founder direct |
...
- **`theverifiedgroup.org` DNS is not wired.** No live URL exists for this repo yet (Phase 0 has no deploy at all — no Vercel config, no `index.html`).
